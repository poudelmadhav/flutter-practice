...
class Employee {
  ...
  factory Employee.fromJson(Map<String, dynamic> json) {
    ...
    return Employee(json['id'], json['employee_name'], json['employee_salary'], json['employee_age'], json ['profileImage']);
  }
...
class ApiWidget extends InheritedWidget {
  ...
  Future<List<Employee>> loadAndParseEmployees() async {
    ...
    if (response.statusCode == 200) {
      ...
      var list = parsed.map<Employee>((json) => Employee.fromJson(json)).toList();
      return list;
    } else {
    ...
    return await showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'Are you sure want to delete ${employee.employeeName}?'
          ),
          actions: <Widget>[
            FlatButton(
              onPressed: () {
...
class _EmployeeDetailState extends State<EmployeeDetailWidget> {
  ...
  _showSnackBar(String content, {bool error = false}) {
    _scaffoldKey.currentState.showSnackBar(SnackBar(
      content: Text('${error ? "An unexpeted error occured: " : ""}$content'),
    ));
  }
    ...
    return TextFormField(
      validator: (value) {
        ...
        if ((salary < 10000) || (salary > 500000)) {
          return 'Please enter salary between 10000 and 500000';
        }
      },
      maxLength: 6,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        padding: EdgeInsets.all(20.0), child: bodyWidget,
      ),
    );
  }
}